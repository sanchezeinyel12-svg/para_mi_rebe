...
(function initMensajeSorpresa() {
  ...
  overlay.style.cssText = `
    position:fixed;inset:0;background:rgba(8,20,32,0.90);
  ...
  style.textContent = `
    ...
    .sorpresa-card-pop{
      ...
      box-shadow:0 30px 80px rgba(13,80,100,0.45);overflow:hidden;
      animation:sorpresaCardIn 0.6s cubic-bezier(.22,1,.36,1) 0.15s both;
    }
    .sorpresa-card-pop::before{
      content:'';position:absolute;top:0;left:0;right:0;height:5px;
      background:linear-gradient(90deg,#c85040,#f4a84a,#1e7a8a);
      border-radius:28px 28px 0 0;
    }
    .sorp-foto{
      width:110px;height:110px;border-radius:50%;object-fit:cover;
      border:4px solid #f4a84a;
      box-shadow:0 6px 24px rgba(244,168,74,0.35);
      margin-bottom:18px;
    }
    ...
    .sorp-titulo{font-family:'Playfair Display',Georgia,serif;font-size:clamp(1.3rem,4vw,1.9rem);
      font-weight:700;color:#0d3848;line-height:1.25;margin-bottom:8px;}
    .sorp-sub{font-family:'Playfair Display',Georgia,serif;font-style:italic;
      font-size:clamp(0.88rem,2vw,1rem);color:#c85040;margin-bottom:22px;line-height:1.6;}
    .sorp-deco{width:48px;height:2px;background:linear-gradient(90deg,#f4a84a,#ffd080);
      border-radius:2px;margin:0 auto 22px;}
    .sorp-msg{font-family:'Playfair Display',Georgia,serif;font-style:italic;
      font-size:clamp(0.95rem,2.2vw,1.08rem);color:#0d3040;line-height:2;margin-bottom:24px;}
    .sorp-firma{font-family:'Caveat',cursive;font-size:1.6rem;color:#1e7a8a;margin-bottom:28px;}
    .sorp-btn{background:linear-gradient(135deg,#c85040,#f4a84a);border:none;
      ...
      box-shadow:0 6px 24px rgba(244,120,60,0.35);
      transition:transform 0.2s,box-shadow 0.2s;}
    .sorp-btn:hover{transform:translateY(-2px) scale(1.04);box-shadow:0 12px 32px rgba(244,120,60,0.4);}
  ...
  overlay.innerHTML = `
    <div class="sorpresa-card-pop">
      ...
      <img class="sorp-foto" src="nv1.jpeg" alt="Foto">
      ...
      <div class="sorp-msg">
   Tu sonrisa me genera muchas cosas mi amor y una de ella son las ganas de seguirte queriendo todos los dias de mi vida ❤️ <br>
      </div>
...
(function initCarta() {
    const TEXTO = `Hola, mi niña chiquita.

Muchas cosas que decirte mi amor, muchas cosas que siente mi corazón por ti.

Desde que llegaste a mi vida todo es más bonito mi amor, me enseñaste a volver amar como nunca antes lo habia hecho, mi amor por ti crece pero demasiado y siempre será asi.

Quiero que sepas que yo solo tengos ojos para ti, no quiero ver otra mujer que no seas tu mi amor, me encantas tu y no sabes cuanto amor

Eres increible mi amor, eres unica las cosas que haces me encantan y como me tratas AMOOOOOO

Te quiero mucho mi niña chiquita eres todo para mi ❤️.


Te quiero hoy, mañana y siempreeeeee. ❤️❤️`;

    const bodyEl = document.getElementById('cartaBody');
    const firmaEl = document.getElementById('cartaFirma');
    const restartBtn = document.getElementById('cartaRestartBtn');

    let idx = 0, timer = null, textNode = null;

    const cursor = document.createElement('span');
    cursor.style.cssText = 'display:inline-block;width:2px;height:1.1em;background:rgba(201,168,76,0.9);vertical-align:middle;margin-left:2px;border-radius:2px;animation:cartaBlink 0.7s steps(1) infinite;';
    const st = document.createElement('style');
    st.textContent = '@keyframes cartaBlink{0%,49%{opacity:1}50%,100%{opacity:0}}';
    document.head.appendChild(st);

    function getDelay(c) {
        if (c === '\n') return 180;
        if (['.', ',', '!', '?'].includes(c)) return 120;
    ...
    function typeNext() {
        if (idx >= TEXTO.length) {
            firmaEl.style.opacity = '1';
            setTimeout(() => { restartBtn.style.opacity = '1'; restartBtn.style.pointerEvents = 'all'; }, 800);
            return;
        }
        textNode.nodeValue += TEXTO[idx++];
        timer = setTimeout(typeNext, getDelay(TEXTO[idx - 1]));
    }

    restartBtn.addEventListener('click', () => { clearTimeout(timer); startTyping(); });
    startTyping();
})();
...
(function initBotella() {
    ...
    // ══════════════════════════════════════════════
    // ✏️  AQUÍ PUEDES EDITAR TUS 30 MENSAJES
    // ══════════════════════════════════════════════
    const MENSAJES = [
        { emoji: "🌊", texto: "Eres lo más bonito que me ha pasado en la vida, gracias por estar a mi lado mi amor." },
        { emoji: "🌅", texto: "Cuando pienso en el futuro que me depara, siempre estás tu en el amor" },
        { emoji: "❤️", texto: "Tu sonrisa es mi lugar favorito en el mundo entero." },
        { emoji: "🦋", texto: "Eres la persona que me hace querer ser mejor versión de mí mismo." },
        { emoji: "🌸", texto: "Me enamoro de ti otra vez cada vez que me miras con esos ojos." },
        { emoji: "✨", texto: "No existe día en el que no piense en ti usted es mi todo." },
        { emoji: "🌙", texto: "Hasta los días malos se vuelven buenos sabiendo que te tengo a ti mi amor" },
        { emoji: "💫", texto: "Quisiera poder regalarte un abrazo cada vez que lo necesitas Te quiero muchote." },
        { emoji: "🌺", texto: "Eres increíblemente única mi amor quisiera que te pudieras ver con los ojos que yo te veo." },
        { emoji: "🎶", texto: "Hay canciones que ya no puedo escuchar sin pensar en ti. Y me encanta eso." },
        { emoji: "🌊", texto: "Contigo aprendí que el amor no siempre daña, aveces cura mi amor." },
        { emoji: "🕯️", texto: "En la oscuridad tú eres mi luz que ilumina mi camino siempre" },
        { emoji: "💌", texto: "Si pudiera mandarte un mensaje cada mañana de mi vida te dijera lo mucho que te quiero mi niña chiquita" },
        { emoji: "🌅", texto: "Ver el atardecer ya nunca será lo mismo sin pensar en ti" },
        { emoji: "🦋", texto: "Gracias por mostrarme que merece la pena abrirse, querer y confiar en alguien." },
        { emoji: "💙", texto: "Eres de esas personas que te cambian la vida sin para bien mi amor" },
        { emoji: "🌿", texto: "Hay una calma especial cuando estoy contigo y quisiera detener el tiempo" },
        { emoji: "⭐", texto: "Si las estrellas tuvieran nombre, la más brillante sería el tuyo mi rebe preciosa." },
        { emoji: "🧡", texto: "Me hace feliz hacerte feliz" },
        { emoji: "🌊", texto: "Te quiero tanto mi amor nunca lo olvides" },
        { emoji: "💍", texto: "Un día quiero que este amor sea nuestra historia favorita para contar a todos." },
        { emoji: "🌸", texto: "Eres mi persona, mi calma, mi aventura y mi todo" },
        { emoji: "✨", texto: "No hay nadie como tú. No la quiero perder" },
        { emoji: "🎶", texto: "Tú eres la razón por la que las canciones de amor cobran sentido." },
        { emoji: "🌙", texto: "Espero que sepas que en mis sueños también apareces huyyyyy amor" },
        { emoji: "💫", texto: "Gracias por recibirme como soy con todo y mis imperfecciones." },
        { emoji: "❤️", texto: "Te quiero con una intensidad que nunca cambiará mi amor." },
        { emoji: "🌺", texto: "Ojalá siempre me mires con esos ojitos que tanto me encantan y amo" },
        { emoji: "🦋", texto: "Mi vida contigo tiene sentido pero demasiado todo es muy especial amor" },
        { emoji: "🌅", texto: "Recuerda siempre que estaré para ti por que me haces muy feliz y no quiero perderte jamás" },
    ];
 
    // Mezclar orden para que sea más sorpresivo
    let indices = [...Array(MENSAJES.length).keys()];
    let vistos  = [];
    ...
    const btnBotella = document.getElementById('botellaBtn');
    const carta      = document.getElementById('botellaCarta');
    const msgEl      = document.getElementById('botellaMsg');
    const emojiEl    = document.getElementById('botellaEmoji');
    const nomEl      = document.getElementById('botellaNomMsg');
    const numEl      = document.getElementById('botellaNum');
    const btnOtra    = document.getElementById('botellaBtnOtra');
 
    function abrirMensaje() {
        if (indices.length === 0) {
            indices = shuffle([...Array(MENSAJES.length).keys()]);
            vistos  = [];
        }
        const idx  = indices.pop();
        const data = MENSAJES[idx];
        vistos.push(idx);
        const num = MENSAJES.length - indices.length;
 
        // Animación botella
        btnBotella.classList.remove('splash');
        void btnBotella.offsetWidth;
        btnBotella.classList.add('splash');
 
        // Actualizar carta
        carta.classList.remove('visible');
        setTimeout(() => {
            emojiEl.textContent  = data.emoji;
            nomEl.textContent    = `Mensaje #${num}`;
            msgEl.textContent    = data.texto;
            numEl.textContent    = num;
            carta.classList.add('visible');
            btnOtra.style.display = 'inline-flex';
        }, 200);
    }
 
    btnBotella.addEventListener('click', abrirMensaje);
    btnOtra.addEventListener('click',   abrirMensaje);
 
    // Partículas
    const sparklesEl = document.getElementById('botellaSparkles');
    for (let i = 0; i < 18; i++) {
        const s = document.createElement('div');
        s.className = 'botella-sparkle';
...
// (function initJardinRecuerdos() {
 
//     const TOTAL = 7;
 
//     /* Coordenadas de cada pétalo alrededor del centro (0,0) */
//     const PETALOS_CFG = [
//         { rot:   0, rx: 15, ry: 32, cx:   0, cy: -40 },
//         { rot:  51, rx: 14, ry: 30, cx:  34, cy: -25 },
//         { rot: 102, rx: 14, ry: 30, cx:  38, cy:  14 },
//         { rot: 153, rx: 15, ry: 31, cx:  12, cy:  42 },
//         { rot: 204, rx: 14, ry: 30, cx: -22, cy:  36 },
//         { rot: 255, rx: 15, ry: 31, cx: -38, cy:   5 },
//         { rot: 306, rx: 14, ry: 30, cx: -26, cy: -32 },
//     ];
 
//     /* Colores de los pétalos: degradan del borgoña al rosa */
//     const COLORES_PETALO = [
//         '#a01848', '#b02050', '#c03060',
//         '#c84070', '#d05080', '#d86090', '#e07090'
//     ];
 
//     const LABELS_PROGRESO = [
//         'el primer pétalo asoma 🌱',
//         'la rosa empieza a nacer 🌸',
//         'ya se empieza a formar 💗',
//         'a mitad del camino 🌹',
//         'casi completa, sigue miamor ✨',
//         '¡un pétalo más y floreceeeee! 💕',
//         'la rosa floreció como nuestro amor cada día mi amorcito 🌹'
//     ];
 
//     /* Elementos */
//     const svg          = document.getElementById('jardinRosaSvg');
//     const petalosGrupo = document.getElementById('jr-petalos');
//     const talloGrupo   = document.getElementById('jr-tallo-g');
//     const centro       = document.getElementById('jr-centro');
//     const centroBrillo = document.getElementById('jr-centro-brillo');
//     const centroLuz    = document.getElementById('jr-centro-luz');
//     const semilla      = document.getElementById('jr-semilla');
//     const progFill     = document.getElementById('jardinProgFill');
//     const progLabel    = document.getElementById('jardinProgLabel');
//     const conteo       = document.getElementById('jardinConteo');
//     const input        = document.getElementById('jardinInput');
//     const btn          = document.getElementById('jardinBtnAgregar');
//     const lista        = document.getElementById('jardinFrasesLista');
//     const frasesTitle  = document.getElementById('jardinFrasesTitulo');
//     const celebracion  = document.getElementById('jardinCelebracion');
//     const celebMsg     = document.getElementById('jardinCelMsg');
//     const btnReset     = document.getElementById('jardinBtnReset');
//     const aura         = document.getElementById('jardinAura');
//     const starsEl      = document.getElementById('jardinStars');
//     const section      = document.getElementById('jardinSection');
 
//     let frases = [];
 
//     /* ── Crear estrellitas de fondo ── */
//     for (let i = 0; i < 30; i++) {
//         const s = document.createElement('div');
//         s.className = 'jardin-star';
//         s.style.cssText = `
//             left: ${Math.random()*100}%;
//             top:  ${Math.random()*100}%;
...
//     /* ── Dibujar un pétalo en el SVG ── */
//     function dibujarPetalo(idx, animado) {
//         const cfg   = PETALOS_CFG[idx];
//         const color = COLORES_PETALO[idx];
 
//         const g = document.createElementNS('http://www.w3.org/2000/svg', 'g');
//         g.setAttribute('transform', `rotate(${cfg.rot})`);
 
//         const ell = document.createElementNS('http://www.w3.org/2000/svg', 'ellipse');
//         ell.setAttribute('cx', cfg.cx);
//         ell.setAttribute('cy', cfg.cy);
//         ell.setAttribute('rx', cfg.rx);
//         ell.setAttribute('ry', cfg.ry);
//         ell.setAttribute('fill', color);
//         ell.setAttribute('stroke', 'rgba(255,255,255,0.12)');
//         ell.setAttribute('stroke-width', '0.8');
 
//         /* Brillito en cada pétalo */
//         const luz = document.createElementNS('http://www.w3.org/2000/svg', 'ellipse');
//         luz.setAttribute('cx', cfg.cx - cfg.rx * 0.25);
//         luz.setAttribute('cy', cfg.cy - cfg.ry * 0.3);
//         luz.setAttribute('rx', cfg.rx * 0.35);
//         luz.setAttribute('ry', cfg.ry * 0.22);
//         luz.setAttribute('fill', 'rgba(255,255,255,0.22)');
...
//             g.style.animation = 'jardinPetaloAparece 0.55s cubic-bezier(0.34,1.56,0.64,1) forwards';
//         }
 
//         petalosGrupo.appendChild(g);
//     }
 
//     /* Inyectar keyframe de pétalo si no existe */
//     if (!document.getElementById('jr-kf')) {
//         const st = document.createElement('style');
//         st.id = 'jr-kf';
//         st.textContent = `
//             @keyframes jardinPetaloAparece {
//                 from { opacity:0; transform: rotate(0deg) scale(0.2); }
//                 to   { opacity:1; transform: rotate(0deg) scale(1); }
//             }
//             @keyframes jardinCentroAparece {
//                 from { r:0; opacity:0; }
//                 to   { opacity:1; }
//             }
//         `;
...
//     /* ── Actualizar la rosa según cantidad de frases ── */
//     function actualizarRosa(n, animarUltimo) {
//         /* Primero dibuja desde 0 si es reset */
//         if (!animarUltimo) {
//             petalosGrupo.innerHTML = '';
//             for (let i = 0; i < n; i++) dibujarPetalo(i, false);
//         } else {
//             dibujarPetalo(n - 1, true);
//         }
 
//         /* Tallo: aparece al llegar a 2+ pétalos */
//         const talloOpacity = Math.min((n - 1) / 2, 1);
//         talloGrupo.style.opacity = n >= 2 ? Math.max(0, talloOpacity).toString() : '0';
//         talloGrupo.style.transition = 'opacity 0.8s ease';
 
//         /* Semilla desaparece al primer pétalo */
//         if (n > 0) semilla.style.display = 'none';
//         else       semilla.style.display = '';
 
//         /* Centro crece proporcionalmente */
//         const radio = Math.min(n * 2.5, 14);
//         const radioBrillo = Math.max(radio - 4, 0);
//         centro.setAttribute('r', radio);
//         centro.setAttribute('opacity', n > 0 ? '1' : '0');
//         centro.style.transition = 'r 0.6s ease, opacity 0.6s ease';
 
//         centroBrillo.setAttribute('r', radioBrillo);
//         centroBrillo.setAttribute('opacity', n > 0 ? '1' : '0');
 
//         centroLuz.setAttribute('rx', Math.max(radio * 0.45, 0));
//         centroLuz.setAttribute('ry', Math.max(radio * 0.3, 0));
//         centroLuz.setAttribute('opacity', n > 0 ? '1' : '0');
 
//         /* Progreso */
//         const pct = Math.round((n / TOTAL) * 100);
//         progFill.style.width = pct + '%';
//         conteo.textContent = n + ' de ' + TOTAL + ' pétalos';
//         if (n > 0) progLabel.textContent = LABELS_PROGRESO[n - 1];
//         else       progLabel.textContent = 'la semilla aún duerme';
 
//         /* Celebración al completar */
//         if (n >= TOTAL) florecerCompleto();
//     }
 
//     /* ── Animación de florecimiento completo ── */
//     function florecerCompleto() {
...
//         const nombres = frases.map((f, i) => '"' + f + '"');
//         celebMsg.innerHTML =
//             'Esta rosa tiene ' + TOTAL + ' pétalos,<br>' +
//             'y cada uno guarda una de sus palabras.<br>' +
//             '<em style="color:rgba(201,168,76,0.8);font-size:0.88em;">' +
//             nombres.slice(0, 3).join(' · ') +
//             (nombres.length > 3 ? ' · ...' : '') +
//             '</em><br><br>' +
//             'Guárdala bien, amor. Es solo tuya. 🌹';
//     }
 
//     /* ── Confeti de partículas ── */
//     function lanzarParticulas() {
//         const colores = ['#c9a84c','#f0d080','#c04070','#e07090','#fff'];
//         const wrap = document.querySelector('.jardin-rosa-svg-cont');
//         for (let i = 0; i < 24; i++) {
//             const p = document.createElement('div');
//             p.className = 'jardin-particula';
...
//         if (!texto || frases.length >= TOTAL) return;
 
//         frases.push(texto);
//         input.value = '';
 
//         /* Mostrar en lista */
//         frasesTitle.style.display = '';
//         const item = document.createElement('div');
//         item.className = 'jardin-frase-item';
//         item.innerHTML =
//             '<span class="jardin-frase-num">0' + frases.length + '</span>' +
//             '<span class="jardin-frase-texto">"' + escapeHtml(texto) + '"</span>';
//         lista.appendChild(item);
//         lista.scrollTop = lista.scrollHeight;
 
//         /* Actualizar rosa con animación */
//         actualizarRosa(frases.length, true);
 
//         /* Deshabilitar si ya completó */
//         if (frases.length >= TOTAL) {
//             btn.disabled = true;
//             input.disabled = true;
//         }
//     }
...
//         frases = [];
//         petalosGrupo.innerHTML = '';
//         semilla.style.display = '';
//         talloGrupo.style.opacity = '0';
//         centro.setAttribute('r', '0');
//         centro.setAttribute('opacity', '0');
//         centroBrillo.setAttribute('r', '0');
//         centroBrillo.setAttribute('opacity', '0');
//         centroLuz.setAttribute('rx', '0');
//         centroLuz.setAttribute('ry', '0');
//         centroLuz.setAttribute('opacity', '0');
//         aura.classList.remove('activa');
//         progFill.style.width = '0%';
//         progLabel.textContent = 'la semilla aún duerme';
//         conteo.textContent = '0 de 7 pétalos';
//         lista.innerHTML = '';
//         frasesTitle.style.display = 'none';
...
//     /* ── Eventos ── */
//     btn.addEventListener('click', agregarFrase);
//     input.addEventListener('keydown', e => {
//         if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); agregarFrase(); }
//     });
...
/* ════════════════════════════════════════════════
   NOTAS DE VOZ INTERACTIVAS (PREGUNTAS)
   ════════════════════════════════════════════════ */
(function initVoiceNotes() {
    const container = document.getElementById('vnContainer');
    if (!container) return;

    // 🎙️ Configura aquí tus audios, títulos, preguntas y respuestas correctas
    // Puedes poner varias respuestas válidas separadas por comas en 'answer'
    const NOTES = [
        { 
            id: 1, src: '1.mp3', label: 'Lo que siento por ti', 
            locked: false // El primero siempre está libre para engancharla
        },
        { 
            id: 2, src: '2.mp3', label: 'Algo que me encanta de ti 🥺', 
            locked: true, question: '¿Cuál es la fecha en la que dijiste que sí?', 
            answer: '17 de abril, 17' 
        },
        { 
            id: 3, src: '3.mp3', label: 'Para mi futura psicóloga 🧠', 
            locked: true, question: '¿Cuál es mi lugar favorito cuando te veo a la cara?', 
            answer: 'tus ojos, los ojos, ojitos' 
        },
        { 
            id: 4, src: '4.mp3', label: 'Una promesa para ti ❤️', 
            locked: true, question: '¿Cómo te digo de cariño?', 
            answer: 'mi niña bonita, amor, amorcito, princesa, lindota' 
        }
    ];
    ...
    NOTES.forEach((note) => {
        // Revisar localStorage para ver si ya lo había desbloqueado antes
        const isUnlocked = localStorage.getItem(`rebeca_vn_${note.id}`) === 'unlocked';
        const effectivelyLocked = note.locked && !isUnlocked;
        const icon = effectivelyLocked ? '🔒' : '▶';

        const el = document.createElement('div');
        el.className = `vn-item ${effectivelyLocked ? 'locked' : ''}`;
        el.id = `vn-item-${note.id}`;

        let challengeHTML = '';
        if (effectivelyLocked) {
            challengeHTML = `
                <div class="vn-challenge" id="vn-challenge-${note.id}">
                    <div class="vn-question">${note.question}</div>
                    <div class="vn-input-row">
                        <input type="text" class="vn-input" id="vn-input-${note.id}" placeholder="Escribe tu respuesta..." autocomplete="off">
                        <button class="vn-submit" id="vn-submit-${note.id}">Abrir</button>
                    </div>
                </div>
            `;
        }
        ...
        el.innerHTML = `
            <div class="vn-main-row" id="vn-row-${note.id}">
                <button class="vn-btn" id="vn-btn-${note.id}">${icon}</button>
                <div class="vn-info">
                    <div class="vn-label">${note.label}</div>
                    <div class="vn-progress-bg">
                        <div class="vn-progress-fill" id="vn-fill-${note.id}"></div>
                    </div>
                </div>
            </div>
            ${challengeHTML}
            <audio id="audio-${note.id}" src="${note.src}" preload="none"></audio>
        `;
        container.appendChild(el);

        if (effectivelyLocked) {
            attachUnlockEvents(note);
        } else {
            attachAudioEvents(note.id);
        }
    });

    // Lógica para desbloquear con la pregunta
    function attachUnlockEvents(note) {
        const itemWrap = document.getElementById(`vn-item-${note.id}`);
        const mainRow = document.getElementById(`vn-row-${note.id}`);
        const input = document.getElementById(`vn-input-${note.id}`);
        const submitBtn = document.getElementById(`vn-submit-${note.id}`);

        // Mostrar/Ocultar la pregunta al tocar la tarjeta
        mainRow.addEventListener('click', () => {
            if (itemWrap.classList.contains('locked')) {
                itemWrap.classList.toggle('show-challenge');
                if (itemWrap.classList.contains('show-challenge')) {
                    input.focus();
                }
            }
        });
        ...
        function verifyAnswer() {
            ...
            const validAnswers = note.answer.split(',').map(a => cleanText(a));

            // Comprueba si alguna de las respuestas válidas está incluida en lo que ella escribió
            const isCorrect = validAnswers.some(valid => userAnswer.includes(valid));

            if (isCorrect) {
                // Desbloquear!
                localStorage.setItem(`rebeca_vn_${note.id}`, 'unlocked');
                
                // Limpiar interfaz
                itemWrap.classList.remove('locked', 'show-challenge');
                itemWrap.classList.add('just-unlocked');
                document.getElementById(`vn-btn-${note.id}`).textContent = '▶';
                
                // Ocultar el div de la pregunta permanentemente
                const challengeDiv = document.getElementById(`vn-challenge-${note.id}`);
                if(challengeDiv) challengeDiv.remove();

                // Lanzar corazones si la función existe
                ...
                void itemWrap.offsetWidth; // reiniciar animación
                itemWrap.classList.add('error');
                input.value = '';
                input.placeholder = "Uy, intenta de nuevo amor...";
            }
        }
    ...
    function attachAudioEvents(id) {
        const btn = document.getElementById(`vn-btn-${id}`);
        const audio = document.getElementById(`audio-${id}`);
        const fill = document.getElementById(`vn-fill-${id}`);
        const itemWrap = document.getElementById(`vn-item-${id}`);

        // Remover el evento de clic anterior si lo tuviera (para evitar doble play)
        ...
        newBtn.addEventListener('click', (e) => {
            ...
            if (currentAudioObj && currentAudioObj !== audio) {
                ...
                const prevBtn = document.getElementById(`vn-btn-${currentPlayingId}`);
                if(prevBtn) prevBtn.textContent = '▶';
                const prevItem = document.getElementById(`vn-item-${currentPlayingId}`);
            ...
            if (audio.paused) {
                audio.play();
                newBtn.textContent = '⏸';
                itemWrap.classList.add('playing');
                currentAudioObj = audio;
                currentPlayingId = id;
            } else {
                audio.pause();
                newBtn.textContent = '▶';
                itemWrap.classList.remove('playing');
            }
        });
    ...
    const POLAROIDS = [
...

        {
        ...
        
         {
            photo: "b1.jpeg",
            emoji: "⭐",
            caption: "Con mi niña bonita❤️",
            note: "Amo cada momento contigo mi preciosaaaaa❤️",
            color: "#e8d8f8",
            rot: "2deg"
        },
        
         {
            photo: "b2.jpeg",
            emoji: "⭐",
            caption: "Mi lugar favorito❤️",
            note: "Eres tu mi niña preciosa❤️",
            color: "#e8d8f8",
            rot: "2deg"
        },
