...
(function initBook() {
  ...
  let currentPage = 0; // cuántas páginas han sido volteadas
  ...
  pages.forEach((data, i) => {
    ...
    if (data.special === 'song') {
      ...
      // Lógica del reproductor del libro
      ...
      return; // skip normal page creation
    }
  ...
  updateUI();
    ...
    /* ══════════════════════════════════════
   CORAZÓN DE FOTOS — script MEJORADO
   Pega esto al FINAL de script.js
   (reemplaza la versión anterior completa)
══════════════════════════════════════ */
...
(function initHeartCanvas() {
  ...
  /* ── Función paramétrica corazón → coordenadas canvas ── */
  // t en [0, 2π], resultado en [0,W] x [0,H]
  function heartPoint(t) {
    const hx = 16 * Math.pow(Math.sin(t), 3);
    const hy = -(13*Math.cos(t) - 5*Math.cos(2*t) - 2*Math.cos(3*t) - Math.cos(4*t));
    // hx en [-16,16], hy en ~[-17,12]
    const pad = W * 0.04;
    return {
  ...
  /* ── Detectar si un punto (px,py) está DENTRO del corazón ──
     Fórmula implícita del corazón: (x²+y²-1)³ - x²y³ ≤ 0
     Mapeamos canvas coords a [-1.2, 1.2] */
  function insideHeart(px, py) {
    const pad = W * 0.04;
    // mapear a [-1.2, 1.2] en x, [-1.3, 1.0] en y
  ...
  /* ── Crear grid de celdas y marcar cuáles están dentro ── */
  const COLS = 5;
  const ROWS = 6;
  const cellW = W / COLS;
  const cellH = H / ROWS;

  // Para cada celda calculamos si su centro está dentro del corazón
  const cells = []; // {col, row, cx, cy, inside}
  ...
  function draw() {
    ...
    /* 3. Pegar fotos en las celdas interiores
          Repetimos fotos si hay más celdas que fotos */
    const gap = 2;
    insideCells.forEach((cell, idx) => {
      ...
      /* drawImage con cover: recortar proporcionalmente */
    ...
    /* 5. Borde brillante doble pasada para glow */
    ...
    /* 6. Destellos dorados en puntos del corazón */
    const glowPoints = [0, 0.25, 0.5, 0.75].map(f => heartPoint(f * Math.PI * 2));
    glowPoints.push({ x: W/2, y: H*0.03 }); // cima
